...
#define USE_HIGHGUI        1 // Enable this to display graph windows using OpenCV's HighGUI. (Supports Windows, Linux & Mac, but not iPhone).
...
// Print the label and then some text info about the IplImage properties, to FACELOG() for easy debugging.
void printImageInfo(const IplImage *image, const char *label)
{
    ...
    // FACELOG can be printf or similar.
    LOGE(s.c_str());
}

// Print the pixel values of the IplImage, to FACELOG() for easy debugging.
        ...
        for (int row = 0; row < image->height; row++)
        {
            ...
            for (int col = 0; col < image->width; col++)
            {
                ...
                for (int ch = 0; ch <= image->nChannels - 1; ch++)
                {
                    ...
                    {
                        // FACELOG can be printf or similar.
                        LOGE("%s ... <just displaying the 1st %d entries from %d!>", s.c_str(), maxElements, image->width * image->height * image->nChannels);
                        return;
                    }
                    ...
                    const int MAX_ELEMENTS_PER_LINE = 60;   // Only print upto 30 numbers per FACELOG() statement, since Android can only handle about 250 characters per log line!
...
// Print the contents of a multi-channel array (using "FACELOG()") for easy debugging.
            ...
            for (int row = 0; row < rows; row++)
            {
                ...
                for (int col = 0; col < cols; col++)
                {
                    ...
                    for (int ch = 0; ch <= channels - 1; ch++)
                    {
                        ...
                        {
                            // FACELOG can be printf or similar.
                            LOGE("%s ... <just displaying the 1st %d entries from %d!>", s.c_str(), maxElements, rows * cols * channels);
                            return;
                        }
                        ...
                        const int MAX_ELEMENTS_PER_LINE = 30;   // Only print upto 30 numbers per FACELOG() statement, since Android can only handle about 250 characters per log line!
                        if (element > MAX_ELEMENTS_PER_LINE)
                        {
                            // FACELOG can be printf or similar.
                            LOGE(s.c_str());
...
// Print the label and then contents of a cv::Mat from the C++ interface (using "FACELOG()") for easy debugging.
// If 'maxElements' is 0, it will print the whole array. If it is -1, it will not print the array at all.
void printMat(const cv::Mat M, const char *label, int maxElements)
{
    ...
    if (!M.empty())
    {
        ...
        // Show the actual data values
        printArray2D(M.data, M.cols, M.rows, channels, depth_type, (int)M.step, maxElements);
    }
    else
    {
        LOGE("%s empty Mat", s.c_str());
    }
}

// Print the label and info of a cv::Mat from the C++ interface (using "FACELOG()") for easy debugging.
...
// Print the label and then contents of a cvMat from the C interface (using "FACELOG()") for easy debugging.
        ...
        for (int row = 0; row < M->rows; row++)
        {
            ...
            for (int col = 0; col < M->cols; col++)
            {
                ...
                for (int ch = 0; ch <= channels - 1; ch++)
                {
                    ...
                    {
                        // FACELOG can be printf or similar.
                        LOGE("%s ... <just displaying the 1st %d entries from %d!>", s.c_str(), maxElements, M->rows * M->cols * channels);
                        return;
                    }
...
void printDataRange(const CvArr *src, const char *msg)
{
    ...
    {  // 2-ch (Complex) input
        ...
        LOGE("\t`` %s Range: Complex MIN = %lf %lfj, MAX = %lf %lfj", msg, min_val[0], min_val[1], max_val[0], max_val[1]);
    ...
    {  // 1-ch (Real) input
        ...
        LOGE("\t`` %s Range: Real MIN = %lf, MAX = %lf", msg, min_val[0], max_val[0]);
    }
    else
    {
        LOGE("\t`` %s Range: UNKNOWN because nChannels == %d != 1 or 2", msg, ((IplImage*)src)->nChannels);
    }
}
...
IplImage* drawFloatGraph(const float *arraySrc, int nArrayLength, IplImage *imageDst, float minV, float maxV, int width, int height, char *graphLabel, bool showScale)
{
    ...
    if (showScale)
    {
        ...
        cvPutText(imageGraph, text, cvPoint(w - b + 4 - 5 * (int)strlen(text), (h / 2) + 10), &font, clr);
    }
...
// Return a new image that is always grayscale, whether the input image was RGB or Greyscale.
// Remember to free the returned image using cvReleaseImage() when finished.
IplImage* convertImageToGreyscale(const IplImage *imageSrc)
{
    IplImage *imageGrey;
    // Either convert the image to grayscale, or make a copy of the existing grayscale image.
    // This is to make sure that the user can always call cvReleaseImage() on the output, whether it was grayscale or not.
...
IplImage* convertImageRGBtoHSV(const IplImage *imageRGB)
{
    ...
    {
        LOGE("ERROR in convertImageRGBtoHSV()! Bad input image.\n");
        exit(1);
    }
    ...
    for (int y = 0; y < h; y++)
    {
        for (int x = 0; x < w; x++)
        {
            ...
            *(pHSV + 0) = (uchar)bH;        // H component
            *(pHSV + 1) = (uchar)bS;        // S component
            *(pHSV + 2) = (uchar)bV;        // V component
        }
    }
    return imageHSV;
}
...
IplImage* convertImageHSVtoRGB(const IplImage *imageHSV)
{
    ...
    {
        LOGE("ERROR in convertImageHSVtoRGB()! Bad input image.\n");
        exit(1);
    }
    ...
    for (int y = 0; y < h; y++)
    {
        for (int x = 0; x < w; x++)
        {
            ...
            uchar *pRGB = (uchar*)(imRGB + y*rowSizeRGB + x * 3);
            *(pRGB + 0) = (uchar)bB;        // B component
            *(pRGB + 1) = (uchar)bG;        // G component
            *(pRGB + 2) = (uchar)bR;        // R component
        }
    }
    return imageRGB;
}
...
IplImage* convertImageYIQtoRGB(const IplImage *imageYIQ)
{
    ...
    {
        LOGE("ERROR in convertImageYIQtoRGB()! Bad input image.\n");
        exit(1);
    }
...
// First create a new (color or grayscale) IPL Image and copy contents of img into it.
...
IplImage* cropImage(const IplImage *img, const CvRect region)
{
    ...
    if (img->width <= 0 || img->height <= 0 || region.width <= 0 || region.height <= 0)
    {
        LOGE("ERROR in cropImage(): dimensions of image or region are invalid.");
        printRect(region, "crop region");
        printImageInfo(img, "crop image");
        exit(1);
    }
...
IplImage* combineImagesResized(int nArgs, ...)
{
    ...
    for (i = 0, m = 20, n = 20; i < nArgs; i++, m += (20 + size))
    {
        ...
        if (img)
        {
            ...
            // Make sure we have a color image. If its grayscale, then convert it to color.
            IplImage *colorImg = 0;
            IplImage *currImg = img;
            if (img->nChannels == 1)
            {
                colorImg = cvCreateImage(cvSize(img->width, img->height), 8, 3);
                //std::cout << "[Converting grayscale image " << greyImg->width << "x" << greyImg->height << "px to color for combineImages()]" << std::endl;
                cvCvtColor(img, colorImg, CV_GRAY2BGR);
                currImg = colorImg;    // Use the grayscale version as the input.
            }
...
IplImage* combineImages(int nArgs, ...)
{
    ...
    for (i = 0; i < nGoodImages; i++)
    {
        ...
        if (img->nChannels == 1)
        {
            colorImg = cvCreateImage(cvSize(img->width, img->height), 8, 3);
            cvCvtColor(img, colorImg, CV_GRAY2BGR);
            img = colorImg;    // Use the grayscale version as the input.
        }
...
IplImage* blendImage(const IplImage* image1, const IplImage* image2, const IplImage* imageAlphaMask)
{
    ...
    for (int y = 0; y < height; y++)
    {
        for (int x = 0; x < width; x++)
        {
            ...
            *pBlended++ = (uchar)(part1B + part2B);
            *pBlended++ = (uchar)(part1G + part2G);
            *pBlended++ = (uchar)(part1R + part2R);
        }
        ...
        pM += widthStepM - width;        // Move to the next Grayscale row, ignoring any row padding.
...
int saveImage(const char *filename, const IplImage *image)
{
    int ret = -1;
#ifdef USE_HIGHGUI
    IplImage *image8Bit = cvCreateImage(cvSize(image->width, image->height), IPL_DEPTH_8U, image->nChannels);    // 8-bit grayscale image.
    if (image8Bit)
...
// Store a grayscale floating-point CvMat image into a BMP/JPG/GIF/PNG image,
...
void saveFloatMat(const char *filename, const CvMat *srcMat)
{
    ...
    {
        LOGE("ERROR in saveFloatMat(): Matrix is not single-channel 32-bit float matrix!");
        return;
    }
...
IplImage* convertFloatImageToUcharImage(const IplImage *srcImg)
{
    ...
    if ((srcImg) && (srcImg->width > 0 && srcImg->height > 0))
    {
        ...
        {
            LOGE("ERROR in convertFloatImageToUcharImage(): Image is not single-channel 32-bit float image!");
            return 0;
        }
        ...
        LOGE("FloatImage:(minV=%lf, maxV=%lf)", minVal, maxVal);
...
{
#ifdef USE_HIGHGUI
    //FACELOG("Saving Float Image '%s' (%dx%d)", filename, srcImg->width, srcImg->height);
    if (srcImg->nChannels != 1 || srcImg->depth != 32)
    {
        LOGE("ERROR in saveFloatImage(): Image is not single-channel 32-bit float image!");
        return;
    }
